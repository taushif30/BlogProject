...
<div className='theLatest'>
    {contextData.filter((item)=> item.id >=25 && item.id<=27).map((item,index)=>{
        return(
   ...
   <div className='topStoriesParent'>
    <div>
        ...
        {contextData.filter((item)=> item.cat === 'Technology').map((item,index)=>{
            return(
    ...
    <div>
        ...
        {contextData.filter((item)=> item.id >=28 && item.id<=30).map((item,index)=>{
            return(
